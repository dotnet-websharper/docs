...
module Client =
    ...
    let Main () =
        ...
        // Create a list model, keyed by itself (will hold unique entries only)
        let myList = ListModel.FromSeq []
        ...
        IndexTemplate.MainTemplate()
            ...
            .AddItem(fun e ->
                ...
                myList.Add(e.Vars.NewItem.Value)
                e.Vars.NewItem.Value <- ""
            )
            .Items(
                myList.View.DocSeqCached(fun (item: string) ->
