...
To integrate WebSharper into your web application, use the `WebSharper.AspNetCore` nuget package.

# Configuring startup

First, we need to set up some services.

- Call `builder.Services.AddWebSharper()` to set up required services (or `services.AddWebSharper()` in the `ConfigureServices` method of your `Startup` class). Any of the following methods also calls this implicitly, so you can use them instead.
- Call `builder.Services.AddSitelet(mySitelet)` to register a WebSharper sitelet.
...
- Add `app.UseWebSharper()` for a basic setup of both remoting, and sitelets if defined in services. It has a builder parameter that can be used for additional configuration:
  ...
  - `UseRemoting(false)` turns off using remoting entirely. `UseRemoting(true, headers)` can also be used to define additional headers to return.
...
- WebSharper also relies on `app.UseStaticFiles()` for serving generated JavaScript.
...
Use this property within your page, like `@Model.MyWebSharperControl`. Do not create WebSharper.Web.Controls directly in your Razor expressions as the WebSharper compiler cannot discover them there for preprocessing.

For a small example, check out https://github.com/dotnet-websharper/aspnetmvc
