...
Forms is an implementation of piglets based on [WebSharper UI](http://github.com/dotnet-websharper/ui). Therefore familiarity with concepts such as `Var`, `View` and the `Doc` type for HTML is necessary to work with it.
